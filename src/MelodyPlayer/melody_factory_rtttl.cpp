...
Melody MelodyFactoryClass::loadRtttlFile(String filepath, FS& fs)
{
    ...
    title.trim();
    if (debug)
        Serial.println(String("Title:") + title);
    if (title.length() == 0)
    {
        return Melody();
    }
    ...
    values.trim();
    if (debug)
        Serial.println(String("Default values:") + values);
    if (values.length() == 0)
    {
        return Melody();
    }
...
Melody MelodyFactoryClass::loadRtttlDB(String filepath, String title, FS& fs)
{
    ...
    {
        if (debug)
            Serial.println("Unable to find melody with title: " + String(title));
        return Melody();
    }
    f.readStringUntil(':');
    ...
    while (f.available() && notes->size() < maxLength && result && bytesUntilNewLine > 0)
    {
        ...
        {
            s = s.substring(0, bytesUntilNewLine);
        }
