...
Melody MelodyFactoryClass::load(String filepath, FS& fs)
{
    ...
    if (nNotes < maxLength)
    {
        ...
        {
            if (debug)
                Serial.println("error during the tokens loading!");
            return Melody();
        }
    }
...
bool MelodyFactoryClass::loadNote(String token)
{
    ...
    if (debug)
        Serial.println(String("note+duration: ") + token);
    ...
    if (noteFormat == NoteFormat::STRING)
    {
        ...
        {
            if (debug)
                Serial.println(String("This note doesn't exist: ") + aux);
            return false;
        }
    }
    ...
    if (debug)
        Serial.println(String("freq: ") + note.frequency);
    ...
    if (debug)
        Serial.println(String("duration: ") + note.duration);
