...
class MelodyPlayer
{
  public:
    /**
     * pwmChannel is optional and you have to configure it only if you play
     * simultaneous melodies.
     */
    MelodyPlayer(unsigned char pin, unsigned char pwmChannel = 0, bool offLevel = HIGH);

    /**
     * Play the last melody in a synchronous (blocking) way.
    ...
    /**
     * Mute the sound of the current melody without stopping it.
     * When unmute will be called the melody will be resumed.
     */
    void mute();
    ...
    /**
     * Change the tempo of the current melody to the provided value.
     */
    void changeTempo(int newTempo);

    /**
     * Set the volume (0-255 value) of the player.
     ...
     * which will produce the highest possible volume for the piezo.
     */
     ...
     * Both players remains independent from each other (e.g. the melody can be independently
  ...
  private:
    ...
    class MelodyState
    {
      public:
        ...
        void saveRemainingNoteDuration(unsigned long supportSemiNote)
        {
            ...
            if (supportSemiNote > now && (supportSemiNote - now) < 10000)
            {
                remainingNoteTime = supportSemiNote - now;
            }
            else
            {
                remainingNoteTime = 0;
    ...
    /**
     * Disable PWM and put the buzzer in low-power state.
     * This call will fail if PWM is not initialized!
     */
    void turnOff();
};
