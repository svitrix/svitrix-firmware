...
namespace {
    uint32_t timerDuration = 0;      // Total duration in seconds
    uint32_t timerRemaining = 0;     // Remaining seconds
    unsigned long timerStartMs = 0;  // millis() when started
...
namespace TimerControl {
    ...
    bool isFinished() { return timerFinished; }
}
...
{
    if (nativeAppGuard("Timer"))
        return;

    uint32_t remaining = TimerControl::getRemaining();

    // Check if timer just finished
    if (timerRunning && remaining == 0 && !timerFinished) {
        timerFinished = true;
        timerRunning = false;
        // TODO: Trigger sound/notification via ISound
    }
...
namespace {
    ...
    unsigned long swAccumulated = 0;  // Accumulated ms when paused
...
namespace StopwatchControl {
    ...
    void pause() {
        if (swRunning) {
            swAccumulated += millis() - swStartMs;
            swRunning = false;
        }
    }
    ...
    uint32_t getElapsed() {
        if (swRunning) {
            return swAccumulated + (millis() - swStartMs);
        }
...
{
    if (nativeAppGuard("Stopwatch"))
        return;

    uint32_t elapsedMs = StopwatchControl::getElapsed();
    uint32_t totalSecs = elapsedMs / 1000;
    uint32_t centisecs = (elapsedMs % 1000) / 10;

    // Format MM:SS.cs (max 99:59.99)
    char timeStr[12];
    if (totalSecs >= 6000) {
        // Cap at 99:59.99
        snprintf(timeStr, sizeof(timeStr), "99:59.99");
    } else {
...
{
    if (nativeAppGuard("Alarms"))
        return;

    // Get alarms from AlarmManager (extern declared in AlarmManager.h)
    ...
    else
    {
        ...
        for (const auto& alarm : alarms)
        {
            ...
            {
                if (!nextAlarm ||
                    (alarm.hour < nextAlarm->hour) ||
                    (alarm.hour == nextAlarm->hour && alarm.minute < nextAlarm->minute))
                {
                    nextAlarm = &alarm;
                }
            }
    ...
    uint16_t textWidth = getTextWidth(displayText.c_str(), 0);
    int16_t textX = (32 - textWidth) / 2;

    DisplayManager.setCursor(textX + x, 6 + y);
    DisplayManager.matrixPrint(displayText.c_str());
}
