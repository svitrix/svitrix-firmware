...
void SlotMachine_::setDisplay(IDisplayRenderer *d)
{
    assert(d);
    display_ = d;
}
bool SlotMachine_::hasDisplay() const
{
...
{
    display_->drawFilledRect(0, 0, 32, REEL_HEIGHT, 0);
    for (int i = 0; i < NUM_REELS; i++)
    {
        drawReel(i);
    }
...
void SlotMachine_::drawReel(int reelIndex)
{
    ...
    display_->drawRGBBitmap(startX, startY + offsetY, currentSymbol, REEL_WIDTH, REEL_HEIGHT);

    if (offsetY > 0)
    {
        const uint32_t *nextSymbol = allSymbols[reelNextSymbols[reelIndex]];
        display_->drawRGBBitmap(
            startX,
