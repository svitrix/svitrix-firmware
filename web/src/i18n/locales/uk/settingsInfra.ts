...
const uk: typeof en = {
  wifi: {
    ...
    confirmConnectBody: "Після підключення до «{{ssid}}» ця сторінка втратить зв'язок і може не повернутися за цією адресою. Годинник покаже свою нову IP-адресу на дисплеї — підключіться за нею.",
  ...
  infra: {
    save: "Зберегти та перезапустити",
    requiresRestart: "Потрібен перезапуск",
    restartHint: "Збереження цих налаштувань перезапускає годинник, і він ненадовго втрачає зв'язок.",
  },
};
