...
| `clients` | array of strings | Allows forwarding a notification to other SVITRIX devices. Use the MQTT prefix for MQTT and IP addresses for HTTP. |  |  | X |
...
| `displayFormat` | string | No | printf-style format with a single conversion (e.g., `"$%.0f"`, `"%.1f°C"`). Allowed: `%d %i %u %o %x %X %f %g %e %E %s` with width and precision modifiers; `%%` for a literal `%`. Other formats are rejected with HTTP 400. | raw value |
...
| `NMODE`       | boolean                   | Enable night mode — scheduled dimming with color override.                                          | `true`/`false`                                     | false   |
| `NSTART`      | integer                   | Night mode start time in minutes from midnight (e.g. 1260 = 21:00).                                | 0–1439                                             | 1260    |
| `NEND`        | integer                   | Night mode end time in minutes from midnight (e.g. 360 = 06:00).                                   | 0–1439                                             | 360     |
| `NBRI`        | integer                   | Brightness during night mode.                                                                        | 0–255                                              | 5       |
| `NCOL`        | integer                   | Text color during night mode (hex as integer, e.g. 16711680 = red).                                 | RGB integer                                        | 16711680 |
| `NBTRANS`     | boolean                   | Block auto-transition during night mode (manual navigation only).                                    | `true`/`false`                                     | true    |
