...
Погодні застосунки також є **вбудованими** (у прошивці), але потребують налаштування **Weather API** (ключ [weatherapi.com](https://www.weatherapi.com/) + місцезнаходження) у **Settings → Weather API**. Після увімкнення вони беруть участь у тому самому циклі ротації, що й інші застосунки, і їх можна вільно впорядковувати (див. *App Order* у посібнику веб-інтерфейсу).

| Застосунок | Опис |
|-----|-------------|
| **Outdoor Temp** | Зовнішня температура. Налаштовуваний колір; враховує шкалу Цельсія/Фаренгейта. |
| **Outdoor Humidity** | Зовнішня відносна вологість (%). Налаштовуваний колір. |
| **Pressure** | Атмосферний тиск (mb/hPa). Налаштовуваний колір. |
| **Air Quality (AQI)** | Індекс якості повітря (US EPA, 1–6). Автоколір за рівнем (зелений → жовтий → помаранчевий → червоний) або фіксований колір. |
| **UV Index** | Ультрафіолетовий індекс (0–11+). Автоколір за рівнем або фіксований колір. |

Кожен погодний застосунок має власну налаштовувану **тривалість** і вмикається окремо. Інтервал оновлення даних задається в розділі Weather API.
