...
| **Display Format** | Необов'язковий формат printf для одного значення (напр., `$%.0f` для "$67432", `%.1f°C` для "23.5°C"). Дозволені специфікатори: `%d %i %u %o %x %X %f %g %e %E %s` з опціональною шириною та точністю (1–2 цифри). Для літерального `%` використовуйте `%%` (напр., `%d%%` → "85%"). |
...
| **Duration** | Тривалість показу у секундах (0 = використовувати глобальний timePerApp) |
