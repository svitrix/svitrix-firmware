...
| `displayFormat` | string | Ні | Рядок формату printf з одним перетворенням (напр., `"$%.0f"`, `"%.1f°C"`). Дозволені: `%d %i %u %o %x %X %f %g %e %E %s` з модифікаторами ширини та точності; `%%` для літерального `%`. Інші формати відхиляються з HTTP 400. | сире значення |
...
| `NMODE`       | boolean                   | Увімкнути нічний режим — затемнення за розкладом з перевизначенням кольору.                          | `true`/`false`                                     | false      |
| `NSTART`      | integer                   | Час початку нічного режиму в хвилинах від опівночі (напр. 1260 = 21:00).                           | 0–1439                                             | 1260       |
| `NEND`        | integer                   | Час закінчення нічного режиму в хвилинах від опівночі (напр. 360 = 06:00).                         | 0–1439                                             | 360        |
| `NBRI`        | integer                   | Яскравість під час нічного режиму.                                                                   | 0–255                                              | 5          |
| `NCOL`        | integer                   | Колір тексту під час нічного режиму (hex як число, напр. 16711680 = червоний).                      | RGB число                                          | 16711680   |
| `NBTRANS`     | boolean                   | Блокувати автоперехід під час нічного режиму (тільки ручна навігація).                              | `true`/`false`                                     | true       |
...
| `BEFF`        | integer                   | Індекс фонового ефекту — відтворюється за кожним застосунком і сповіщенням. Використовуйте `-1`, щоб вимкнути. | `-1` або дійсний індекс ефекту (див. `/api/effects`) | -1     |
