...
| `displayFormat` | string | Ні | Рядок формату printf з одним перетворенням (напр., `"$%.0f"`, `"%.1f°C"`). Дозволені: `%d %i %u %o %x %X %f %g %e %E %s` з модифікаторами ширини та точності. Інші формати відхиляються з HTTP 400. | сире значення |
